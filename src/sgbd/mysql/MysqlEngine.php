...
class MysqlEngine extends \classes\Interfaces\resource implements DatabaseInterface {
    ...
    private function connect($bd_server, $bd_name, $bd_user, $bd_password) {
        ...
        try {
            ...
            if (!file_exists($file))
                throw new \classes\Exceptions\DBException("Classe ($class) não encontrada");
...
//            echo "($query - $fetch)<br/>\n\n";
    ...
    private function setQuery($query) {
        ...
        if ($this->is_transaction) {
            ...
            $this->sentenca = implode(";", $this->query) . ";";
        }
    }
    ...
    private function generateErrorMessage($e) {
        ...
        if (DEBUG) {
            ...
            if (trim($sentenca) != '')
                $debug .= "<b>Sentença sql</b>: <br/>$sentenca<br/><br/>";
            $debug .= "</div>
                <hr/>";
        }
        $arr['erro'] = $debug;
    ...
    private function LoadErrorFromClass($erro, $state, $msg, $die = false) {
        ...
        if (!$this->LoadResourceFile($file, false))
            return "";
    ...
    public function importDataFromArray($dados, $tabela, $callback = null, $insertIgnore = false, $callbackArgs = array(), $callbackStatementsFn = null) {
        ...
        $cols = implode("`,`", $keys);
        $update = "";
        if ($callbackStatementsFn !== null) {
            $update = $callbackStatementsFn($keys);
        }
        if (trim($update) == "") {
